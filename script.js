...
// ══════════════════════════════════════════
// 🔑 API KEY MANAGER (localStorage, never in code)
// ══════════════════════════════════════════
const LS_APIKEY    = "epai_api_key";
const LS_PROVIDER  = "epai_provider";
...
const PROVIDERS = {
  ...
  anthropic: {
    ...
    parseResponse: (data) => data.content?.map(b => b.text || "").join("").trim() || "",
  },
  gemini: {
    name: "Gemini",
...
let currentProvider = localStorage.getItem(LS_PROVIDER) || "groq";

function getApiKey() {
  return localStorage.getItem(LS_APIKEY + "_" + currentProvider) || "";
}

// Auto-save key as user types (debounced 600ms)
...
function onApiKeyInput() {
  ...
  _autoSaveTimer = setTimeout(() => {
    ...
    if (key.length > 10) {
      localStorage.setItem(LS_APIKEY + "_" + currentProvider, key);
      updateApiKeyStatus();
      const msg = document.getElementById("apiKeyMsg");
      msg.textContent = "✓ Tersimpan otomatis";
      msg.className = "lock-msg ok";
      setTimeout(() => { msg.textContent = ""; msg.className = "lock-msg"; }, 2000);
    }
  }, 600);
}
...
function saveApiKey() {
  ...
  const msg = document.getElementById("apiKeyMsg");
  if (!key) {
    msg.textContent = "Masukkan API key dulu";
    msg.className = "lock-msg err";
    return;
  }
  localStorage.setItem(LS_APIKEY + "_" + currentProvider, key);
  const preview = key.slice(0, 6) + "..." + key.slice(-4);
  msg.textContent = "✓ Tersimpan: " + preview;
  msg.className = "lock-msg ok";
  document.getElementById("apiKeyInput").value = "";
  updateApiKeyStatus();
  setTimeout(() => { msg.textContent = ""; msg.className = "lock-msg"; }, 3000);
}

function clearApiKey() {
  localStorage.removeItem(LS_APIKEY + "_" + currentProvider);
  updateApiKeyStatus();
  const msg = document.getElementById("apiKeyMsg");
  msg.textContent = "Key dihapus";
  msg.className = "lock-msg err";
  setTimeout(() => { msg.textContent = ""; msg.className = "lock-msg"; }, 1500);
}
...
function switchProvider(provider) {
  ...
  const linkEl = document.getElementById("apiGetLink");
  if (linkEl) linkEl.href = PROVIDERS[provider].keyLink;
  const inputEl = document.getElementById("apiKeyInput");
  if (inputEl) inputEl.placeholder = PROVIDERS[provider].name + " key (" + PROVIDERS[provider].keyHint + ")";
  updateApiKeyStatus();
}
...
function updateApiKeyStatus() {
  ...
  if (key) {
    ...
    statusEl.textContent = "Belum ada key";
    statusEl.style.color = "rgba(212,175,55,0.4)";
...
function startAccessSequence() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      // Update sidebar model label
      const p = PROVIDERS[currentProvider];
      if (p) document.getElementById("modelLabel").textContent = p.name + " · " + p.model.split("/").pop().split(":")[0];
    }, 600);
  }, 3500);
}
...
async function sendMessage() {
  ...
  try {
    ...
    const response = await fetch(provider.url, {
      ...
      body: JSON.stringify(provider.buildBody(apiMessages, CONFIG.SYSTEM_PROMPT)),
    });
    ...
    const aiText = provider.parseResponse(data);
    ...
    if (err.message === "NO_KEY") {
      const p = PROVIDERS[currentProvider];
      fallback = `⬡ **API key belum diatur** — Kembali ke lock screen dan masukkan API key kamu.\n\nDapatkan key gratis di: ${p.keyLink}`;
    } else {
      fallback = `⬡ **Connection error** — ${err.message}\n\nCek kembali API key kamu atau coba provider lain.`;
    }
...
function buildApiMessages(messages, currentUserMsg) {
  ...
  const mapped = history.map(m => {
    ...
    if (m.role === "user") {
      ...
      if (m.attachment && m.attachment.type === "file" && m.attachment.textContent) {
        const text = (`[File: ${m.attachment.name}]\n\n${m.attachment.textContent}\n\n${m.content}`).trim();
        return { role: "user", content: text || "." };
      }
      // Plain text (skip image for Groq/OpenRouter compatibility)
  ...
  // Filter out any empty content and ensure no consecutive same-role messages
  const filtered = [];
